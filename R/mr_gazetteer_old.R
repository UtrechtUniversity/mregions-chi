...
mr_gazetteer_feature_get <- function(mrid) {
  ...
  features <- do.call(rbind, features)}
    ...
    print(paste(mrid, "WARNING, composed of multiple features, now selecting last feature for processing"))
    features <- aggregate(features, fun=mean())
    #features <- do.call(rbind, list(features[[2]]))
    #features <- do.call(rbind, list(features[[2]]))
  }
  features
}
